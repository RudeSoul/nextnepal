...
import { Row, Col, Container, Form, Button, Tab, Nav, Image, Badge, Card } from 'react-bootstrap';
...
class ServiceDetail extends React.Component {
	...
	componentDidMount() {
		...
		instance.post(`get-store-services/${this.props.match.params.slug}`).then(data => {
			this.setState({
			...
			<>
				<section className="restaurant-detailed-banner">
					<div className="text-center">
						<Image fluid className="cover" src={this.state.res.banner_image? process.env.REACT_APP_IMAGE_URL + this.state.res.banner_image : "/img/mall-dedicated-banner.jpg"} />
					</div>
							...
							<Row className="d-flex align-items-end">
								<Col md={8}>
									<div className="restaurant-detailed-header-left">
										<Image fluid className="mr-3 float-left" alt="osahan" src={process.env.REACT_APP_IMAGE_URL + this.state.res.image} />
										<h2 className="text-white">{this.state.res.name}</h2>
										<p className="text-white mb-1"><Icofont icon="location-pin" /> {this.state.res.address}<Badge variant="success">{this.state.res.is_active ? 'OPEN' : 'CLOSED'}</Badge>
										</p>
										<p className="text-white mb-0"><Icofont icon="food-cart" /> {this.state.res.description}
										</p>
									</div>
								</Col>
								<Col md={4}>
									<div className="restaurant-detailed-header-right text-right">
										<Button variant='success' type="button"><Icofont icon="clock-time" /> {this.state.res.delivery_time} min
	                        </Button>
							...
							<Row>
									...
									<div className="offer-dedicated-body-left">
										<Tab.Content className='h-100'>
											<Tab.Pane eventKey="first">
												...
												{/* <Row>
													<h5 className="mb-4 mt-3 col-md-12">Recommended <small className="h6 text-black-50">{this.state.recommended.length} ITEMS</small></h5>
													<Col md={12}>
														<div className="bg-white rounded border shadow-sm mb-4">
															{
																this.state.recommended.map(rec => (
																	<QuickBite
																		...
																		itemClass="menu-list"
																		title={rec.name}
																		image={rec.image ? process.env.REACT_APP_IMAGE_URL + rec.image : null}
												...
												</Row> */}
												{
													this.state.items &&  Object.keys(this.state.items).map((key) => (
														<Row key={key}>
															<h5 className="mb-4 mt-3 col-md-12">{key} <small className="h6 text-black-50">{this.state.items[key].length} Service</small></h5>
															<Col md={12}>
																<div className="bg-white rounded border shadow-sm mb-4">
																	{
																		this.state.items[key].map(itm => (
																			<QuickBite
																				...
																				id={itm.id}
																				itemClass="menu-list"
																				image={itm.image ? process.env.REACT_APP_IMAGE_URL + itm.image : 'img/1.jpg'}
																				title={itm.name}
											...
											<Tab.Pane eventKey="third">
												<div id="restaurant-info" className="bg-white rounded shadow-sm p-4 mb-4">
													...
													<p className="mb-3">{this.state.res.address}, {this.state.res.pincode}</p>
								...
								<Col md={4}>
									...
									<div className="generator-bg rounded shadow-sm mb-4 p-4 osahan-cart-item">

										{
											(this.props.cart.length > 0) ?
												<React.Fragment>
													...
													<p className="mb-4 text-white">{this.props.cart.length} Items</p>
												...
												<Card className="mb-2 p-2">
													...
													<p>Cart item is empty. You can select the food item and choose addons.</p>
												</Card>
										}
										{
											this.props.cart.length > 0 &&
										...
										<h6 className="font-weight-bold text-right mb-2">Subtotal : <span className="text-danger">Rs.{this.props.total}</span></h6>
													<p className="seven-color mb-1 text-right">Extra charges may apply</p>
												</div>
												<Link to="/" onClick={this.handleCheckout} className="btn btn-success btn-block btn-lg">Checkout
